...
关于项目二（交互式绘图系统）的 AI 协作分析
在开发交互式绘图应用程序（Project 2）的过程中，我深入使用了 AI 助手辅助开发。针对该项目的特点（重 GUI 交互、重面向对象设计），AI 的表现如下：

✅ 1. 核心优势 (Strengths)
加速 MFC 消息映射与框架搭建： MFC 的消息映射宏（如 ON_WM_LBUTTONDOWN、ON_WM_MOUSEMOVE）和类结构较为繁琐。AI 能够迅速生成标准的框架代码，帮助我快速完成了从 CView 到具体交互函数的脚手架搭建，节省了大量查阅文档的时间。

攻克图形算法难点（双缓冲与橡皮筋技术）： 在实现“无闪烁绘图”时，AI 提供的双缓冲技术 (Double Buffering) 方案（创建兼容 DC 和位图）非常精准，一次性解决了屏幕闪烁问题。此外，在实现鼠标拖拽绘制形状时的橡皮筋效果 (Rubber-banding) 逻辑上，AI 清晰地解释了 SetROP2(R2_NOT) 的异或绘图原理，帮助我实现了流畅的动态预览。

面向对象设计的落地： 项目要求通过多态管理不同的图元（直线、矩形、椭圆）。AI 辅助我设计了 CShape 基类及其虚函数接口（Draw, IsSelected），并指导我使用 std::vector<CShape*> 或 CObArray 进行统一管理，强化了我对 C++ 多态性的理解。

❎ 2. 局限与不足 (Limitations)
可视化资源编辑的“盲区”： 绘图应用涉及大量的 UI 控件（工具栏图标、菜单栏、Ribbon 界面）。AI 无法操作 Visual Studio 的资源视图 (Resource View)，无法帮我拖拽控件或修改 .rc 资源文件。所有的图标 ID 绑定和菜单布局都需要我手动完成，AI 只能提供代码层面的逻辑，无法解决“所见即所得”的 UI 设计问题。

序列化 (Serialization) 的版本兼容性问题： 在实现文件保存（CArchive）功能时，AI 有时会混淆 MFC 的旧式序列化宏（DECLARE_SERIAL）与现代 C++ 的流式操作，导致代码风格不统一。特别是在处理对象指针数组的存储和读取时，AI 生成的代码曾引发读取崩溃，最终通过查阅微软官方文档才修正了对象重建的逻辑。

复杂交互状态的上下文丢失： 绘图操作极其依赖状态机（如：空闲 -> 鼠标按下 -> 拖动中 -> 鼠标松开）。当我在长对话中反复修改某个状态的逻辑时，AI 容易“忘记”之前的上下文，导致生成的代码覆盖了之前的边界检查（例如忘记在 OnLButtonUp 中重置 m_bDrawing 标志位），需要我人工进行逻辑缝合与排查。

💡 总结 (Takeaway)
在项目二中，AI 是完美的**“算法导师”和“代码生成器”，特别是在处理 GDI 绘图逻辑和类结构设计上表现优异；但在涉及UI 资源管理和复杂状态流转**时，仍需开发者具备清晰的逻辑判断能力和手动操作能力。